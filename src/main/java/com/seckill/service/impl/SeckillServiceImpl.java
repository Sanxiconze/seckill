...
public class SeckillServiceImpl implements SeckillSerivce {
    ...
    //md5盐值字符串，用于生成md5
    private final String slat = "asdfjasdfanwerhuxcxvdsfajkl3j4234@#%@#%&^*$%^&!fWK3km";
     ...
     * 2.保证事务方法的执行时间尽可能短。不要穿插其他网络操作RPC/HTTP请求或者剥离道事务方法外
    ...
    public SeckillExcution executeSeckill( long seckillId, long userPhone, String md5 )
        ...
        try {
             ...
             **/
            /*
            int updateDate = seckillDao.reduceNumber(seckillId, nowTime);
            if (updateDate <= 0) {
                //没有更新操作
            ...
            /**
             * 优化后: insert不触发行级锁，可以并发执行，并且在插入失败后不执行update
             * 优化前：update时触发行级锁，在函数执行完毕后会自动commit，在事务中既有update又有insert，会执行两条命令，并且有两倍的网络延迟
             */
            int insertCount = successKilledDao.insertSuccessKilled(seckillId, userPhone);
            if (insertCount <= 0) {
                throw new RepeatKillException("seckill repeated");
